...
description: اسکریپت پایه برای اضافه کردن رنگ‌های بیشتر بازیکن.
---

این آموزش برای استفاده با [GetPlayerColor](../scripting/functions/GetPlayerColor) است، اگر از [SetPlayerColor](../scripting/functions/SetPlayerColor) در اسکریپت خود هنگام اتصال بازیکنان استفاده نمی‌کنید.
...
ابتدا این را در بالای اسکریپت خود قرار دهید.

سپس این را زیر callback OnPlayerConnect قرار دهید:

```pawn
SetPlayerColor(playerid, PlayerColors[playerid % sizeof PlayerColors]);
```

حالا [GetPlayerColor](../scripting/functions/GetPlayerColor) کار خواهد کرد!

برای نسخه‌های جدید SA-MP می‌توانید این آرایه را اضافه کنید:
...
حالا رنگ‌های تکراری وجود ندارد، رنگ‌ها به حداکثر تعداد slotها توزیع شده‌اند، گام رنگ - 34، و TAB سرور پر مثل رنگین‌کمان به نظر می‌رسد!