...
public OnPlayerCommandText(playerid, cmdtext[])
{
    ...
    {
        // 打开并让 NPC 0 以慢跑速度和自定义半径巡逻节点 15
        if (NPC_OpenNode(15))
        {
            NPC_PlayNode(0, 15, NPC_MOVE_TYPE_JOG, 1.5, 2.0, true);
            SendClientMessage(playerid, 0x00FF00FF, "NPC 0 正在巡逻节点 15");
        }
        else
        {
            SendClientMessage(playerid, 0xFF0000FF, "打开节点 15 失败");
        }
        return 1;
    }
    ...
    {
        // 打开并让 NPC 步行通过节点 8，不改变角度
        if (NPC_OpenNode(8))
        {
            NPC_PlayNode(0, 8, NPC_MOVE_TYPE_WALK, NPC_MOVE_SPEED_AUTO, 0.0, false);
            SendClientMessage(playerid, 0x00FF00FF, "NPC 0 正在步行节点 8");
        }
        return 1;
    }
    return 0;
...
{
    printf("NPC %d 完成导航节点 %d", npcid, nodeId);

    // 开始导航下一个节点
    new nextNode = nodeId + 1;
    if (nextNode <= 63) // 节点 ID 为 0-63
    {
