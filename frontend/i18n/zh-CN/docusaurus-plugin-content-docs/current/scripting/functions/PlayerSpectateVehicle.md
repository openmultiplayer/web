...
使玩家进入观战模式并附着视角到指定车辆，获得类似驾驶位的观察视角

| 参数               | 说明                                                             |
| ------------------ | ---------------------------------------------------------------- |
| playerid           | 观战者玩家 ID                                                    |
| targetvehicleid    | 被观战车辆 ID                                                    |
| SPECTATE_MODE:mode | [观战模式](../resources/spectatemodes)（可选参数，默认普通模式） |

## 返回值

**true** - 指令执行成功（注意：未启用观战模式时也会返回成功，但实际无效，必须先调用 TogglePlayerSpectating）
...
```c
TogglePlayerSpectating(playerid, 1);         // 激活观战模式
...
- [观战模式详解](../resources/spectatemodes)
