...
此时`print`函数在脚本层面已不存在。虽然 SA:MP 底层仍保留此函数，且编译器通过"= print"知晓其真实名称，但在 PAWN 代码中调用`print()`将报错，因其已被重命名为`my_print`。鉴于`print`的脚本级定义已移除，开发者可自由重新定义：
...
`public`关键字用于使函数对虚拟机可见，即允许 SA:MP 服务器直接调用此函数，而不仅限于在 PAWN 脚本内部调用。该关键字也可用于变量声明，使其值能在服务器端读写(尽管 SA:MP 未直接使用此特性，但可通过插件实现)，结合`const`可创建仅允许服务器修改的只读变量。
...
`stock` 用于声明可能未被使用但你不希望生成未使用警告的变量和函数。对于变量而言，`stock` 类似于 `const`，它是一个修饰符而非完整声明，因此你可以这样使用：
...
`stock` 最常见于自定义库开发场景。当你编写函数库时，会提供大量函数供他人使用，但你无法预知哪些函数会被实际调用。如果用户未使用的每个函数都会触发大量警告，他们可能会提出抱怨（除非某些函数必须被调用以执行初始化变量等强制操作）。尽管如此，根据 YSI 库的实际经验，用户往往仍会提出各种反馈。
...
在此示例中，`Func2` 从未被调用，因此编译器会生成警告。这在独立脚本中可能有助于提醒你是否遗漏了函数调用，但如果 `Func1` 和 `Func2` 属于函数库，用户可能确实不需要 `Func2`。此时可采用以下方案：
