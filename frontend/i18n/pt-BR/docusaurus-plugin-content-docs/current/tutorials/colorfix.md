...
sidebar_label: "Correção de Cores"
descripion: Um script básico para adicionar mais cores de jogadores.
---

Este tutorial deve ser usado com [GetPlayerColor](../scripting/functions/GetPlayerColor), se você não usar [SetPlayerColor](../scripting/functions/SetPlayerColor) no seu script quando os jogadores se conectam.
...
Primeiro coloque isso no topo do seu script.

Em seguida, coloque isso sob o callback OnPlayerConnect:

```pawn
SetPlayerColor(playerid, PlayerColors[playerid % sizeof PlayerColors]);
```

Agora [GetPlayerColor](../scripting/functions/GetPlayerColor) funcionará!

Para novas versões do SA-MP você pode adicionar este array:
...
Agora não há cores duplicadas, as cores são distribuídas para o número máximo de slots, o passo de cor - 34, e a TAB do servidor cheio parece um arco-íris!
