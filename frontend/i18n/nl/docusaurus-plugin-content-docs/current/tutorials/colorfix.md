...
description: Basis script om meer player-kleuren toe te voegen.
---

Deze tutorial gebruik je samen met [GetPlayerColor](../scripting/functions/GetPlayerColor), als je niet [SetPlayerColor](../scripting/functions/SetPlayerColor) aanroept wanneer spelers connecten.
...
Plaats dit bovenaan je script.

Plaats vervolgens dit in `OnPlayerConnect`:

```pawn
SetPlayerColor(playerid, PlayerColors[playerid % sizeof PlayerColors]);
```

Nu werkt [GetPlayerColor](../scripting/functions/GetPlayerColor) correct!

Voor nieuwere SA-MP-versies kun je deze array toevoegen:
