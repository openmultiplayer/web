...
- Если вы хотите установить на сервер пароль, чтобы войти могли только довереннеыелюди, добавьте следующую строку в [server.cfg](server.cfg):
...
Вы можете авторизоваться, как RCON администратор, при помощик команды `/rcon login password` в игре, или используя [Удалённую Консоль](remoteconsole).
...
Чтобы заблокировать оперелённый IP адрес, используя полномочия RCON администратора, необходимо использовать команду `banip`.
...
| `/rcon rcon\_password [PASSWORD]` | Меняет пароль RCON админисраторов                                                                                                    |
...
Первые 4 вышеприведённых командя для 0.3z R2 были сделаны для противодействия атакам на серверы SA-MP, вызывающие фризы и сбои. Поэтому, используйте их на своём сервере. Значения по умолчанию должны быть корректными для большинства серверов, однако, если вы замечаете ложные кики из-за этого, просто повысьте значения, пока невинные игроки не были исключены/хаблокированы. [Подробнее здесь](https://web.archive.org/web/20190426141744/http://forum.sa-mp.com/showpost.php?p=2990193&postcount=47)/
...
- [OnRconLoginAttempt](../../callbacks/OnRconLoginAttempt): Вызывается, когда совершается попытка авторизации RCON администраора.
